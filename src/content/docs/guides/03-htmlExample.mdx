...
En ocaciones los caracteres especiales pueden causar errores en la pagina web,como ser `á`, `é`, `í`, `ó`, `ú`, `ñ`, etc.ara evitar estos errores se recomienda usar la codificacion `UTF-8` en el documento HTML. Para esto usamos la etiqueta `<meta>` con el atributo `charset="UTF-8"` dentro de la etiqueta `<head>`.
```html ins={2}
<head>
    <meta charset="UTF-8">
</head>
...
## Trabajo en clase 1 :badge[Actividad]{variant=caution}
